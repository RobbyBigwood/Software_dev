...
class RobotWorld:
    ...
    def __init__(self, grid_size = 7, pit_percent = 10):
        ...
        self.grid = [[0 for i in range(self.board_size)] for j in range(self.board_size)]
       
        # size x size grid spaces (make pit_percent of those into pits)
        self.pit_count = int(pit_percent / 100 * self.board_size * self.board_size)
        ...
        self.die_cost = self.board_size * self.board_size + 1
        self.last_move = 0
        ...
        self.starter = [random.choice(MOVE_CHOICES)
                        for i in range(self.board_size * self.board_size - self.pit_count - 2)]
    ...
    def generate_neighbor(self, solution):
        ...
        #Timothy solution
        if strategy == 1:
            # Change a single random move
            index = random.randint(0, len(neighbor) - 1)
            neighbor[index] = random.choice(MOVE_CHOICES)
        elif strategy == 2:
        ...
        #Robby solution
            ...
            var = random.randint(0, len(neighbor) - 1)
            neighbor[var] = random.choice(MOVE_CHOICES)
        return neighbor
