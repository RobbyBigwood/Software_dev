...
student_count = 20
course_count = 5
courses = [chr(c) for c in range(ord("A"),ord("A")+course_count)]
...
for line in student_info:
    ...
    students.append(student.Student(lister,courses))

#for s in students:
#    print(s)
matplotlib.use('TkAgg')

best_generations,avg_generations = ga.solve(students,courses)
plt.plot(list(range(500)), avg_generations, label='average')
plt.plot(list(range(500)), best_generations, label='best')
plt.xlabel("Generation #")
plt.ylabel("Generation Avg Value")
plt.title("Generation Values over Iterations")
plt.legend()
plt.show()
