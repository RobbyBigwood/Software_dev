...
def solve(students,courses):
    ...
    for i in range(generations):
        ...
        avg_generations.append(sum(population_fitness)/len(population_fitness))
        ...
        best_generations.append(best_fitness)
        ...
        # Original next_gen.py
        #population = next_gen_alt.next_gen(population,population_fitness)
        # Ben's new/alternative next_gen_alt.py for issue 6
        population = next_gen_alt.next_gen(population,population_fitness)
    ...
    print(f'Population fitness is {avg_generations}')
    print(f'Population all best is {best_generations}')
    # print the solution
    print_roster(best_schedule, students, courses)
    #print the graph?
    return best_generations, avg_generations

def print_roster(schedule,students, courses):
    # create a dictionary to hold the list of students enrolled in each course
    rosters = { chr(c):[] for c in range(ord("A"),ord("A")+len(courses))}
